...
import com.aizhixin.cloud.dataanalysis.simulate.service.AddAttendanceDataService;
...
public class TestAnalysisDataController {
    ...
    @Autowired
    private AddAttendanceDataService addAttendanceDataService;
...
//			}
    ...
    @GetMapping(value = "/addattendancedata", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(httpMethod = "GET", value = "手动添加考勤数据", response = Void.class, notes = "手动添加考勤数据<br><br><b>@author dengchao</b>")
    public Map<String,Object> addAttendanceData(
            @ApiParam(value = "orgId 机构id", required = true) @RequestParam(value = "orgId", required = true) Long orgId,
            @ApiParam(value = "collegeCode 学院code", required = true) @RequestParam(value = "collegeCode", required = true) String collegeCode,
            @ApiParam(value = "professionCode 专业code", required = false) @RequestParam(value = "professionCode", required = false) String professionCode,
            @ApiParam(value = "className 班级名称", required = false) @RequestParam(value = "className", required = false) String className,
            @ApiParam(value = "xnxq 学年学期 格式如:2017-2018-2", required = true) @RequestParam(value = "xnxq", required = true) String xnxq
    ) {
        return addAttendanceDataService.addDate(orgId, collegeCode,professionCode,className, xnxq);
    }
}